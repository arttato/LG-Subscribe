...
- ดึง 5 อย่างต่อรุ่น:
  - `features` — คุณลักษณะที่สำคัญ (bullet ย่อ — ส่วนเดิมบนหน้า detail)
  - `keySpecs` — ตารางแรกของหน้า (สเปคเด่น)
  - `featureBlocks` — **คุณสมบัติพร้อมรูป** (ช่วง pdp-overview-section ถึง pdp-support-section, เฉพาะ c-folding block: หัวข้อ + คำอธิบาย + รูป desktop)
  - `groups` — **สเปคทั้งหมด** (ทุกกลุ่มใน c-all-specs-area: ชื่อกลุ่ม + ตารางชื่อ/ค่า)
  - `faq` — **คำถามที่พบบ่อย** (จาก JSON-LD FAQPage)
- ข้อมูลที่ดึงได้เขียนไป `src/data/lg-specs.json` แล้ว commit — หน้า detail อ่านจากไฟล์นี้ (โหลดตอน build)
- หน้า detail แสดงส่วนเพิ่มเติม **ใต้ส่วนบริการดูแล**: คุณสมบัติ (รูปสลับซ้าย/ขวา) → สเปคทั้งหมด (กลุ่มเปิดปิดได้) → คำถามที่พบบ่อย (accordion) — รุ่นไหนที่หน้า LG ไม่มีส่วนนั้น จะไม่แสดงส่วนนั้น (เช่น แอร์บางรุ่นไม่มีคุณสมบัติพร้อมรูป)
